...
const DeleteVideoFromUpload = ({ name }) => {
  ...
  const deleteVideoFromUpload = async () => {
    try {
      const data = await Chapar.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/file/video/${name}`,
      )
...
export default DeleteVideoFromUpload
